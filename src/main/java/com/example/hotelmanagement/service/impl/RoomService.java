...
public Response addNewRoom(RoomDTO roomDTO) {
    ...
    try {
        ...
        room.setRoomPhotoUrl(roomDTO.getRoomPhotoUrl()); // can be null

        Room savedRoom = roomRepository.save(room);
        RoomDTO savedDTO = Utils.mapRoomEntityToRoomDTO(savedRoom);

        response.setStatusCode(200);
        response.setMessage("Successful");
        response.setRoom(savedDTO);
    } catch (Exception e) {
        response.setStatusCode(500);
        response.setMessage("Error saving a room: " + e.getMessage());
    }
    ...
    public Response getAvailableRoomsByDateAndType(LocalDate checkInDate, LocalDate checkOuDate, String roomType) {
        Response response=new Response();
        try{
           List<Room> availableRooms= roomRepository.findAvailableRoomsByDatesAndTypes(checkInDate, checkOuDate, roomType);
