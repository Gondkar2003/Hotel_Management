...
public ResponseEntity<Response> addNewRoom(@RequestBody RoomDTO roomDTO) {
    ...
    if (roomDTO.getRoomType() == null || roomDTO.getRoomType().isBlank() || roomDTO.getRoomPrice() == null) {
        Response response = new Response();
        response.setStatusCode(400);
        response.setMessage("Please provide values for roomType and roomPrice");
        return ResponseEntity.status(response.getStatusCode()).body(response);
    }

    // Call service method that accepts RoomDTO
    ...
    return ResponseEntity.status(response.getStatusCode()).body(response);
	    ...
	    @PreAuthorize("hasAuthority('ADMIN')")
	    public ResponseEntity<Response> updateRoom(@PathVariable Long roomId, @RequestBody RoomDTO roomDTO){
